...
<div class="grid min-w-0 gap-5 xl:grid-cols-[340px_minmax(0,1fr)]">
  <aside class="h-fit min-w-0 overflow-hidden rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900 xl:sticky xl:top-28">
    ...
    <form class="mt-5 grid min-w-0 gap-4" onsubmit={submit}>
      <label class="grid min-w-0 gap-1.5 text-sm font-bold text-slate-600 dark:text-slate-300">
        검색어
        <div class="flex min-w-0 overflow-hidden rounded-xl border border-slate-200 bg-slate-50 focus-within:border-emerald-500 dark:border-slate-700 dark:bg-slate-800">
          <select bind:value={match} aria-label="검색 방식" class="w-24 shrink-0 border-r border-slate-200 bg-transparent px-2 text-xs font-bold outline-none dark:border-slate-700 dark:text-white">
            <option value="CONTAINS">포함</option>
            <option value="EXACT">일치</option>
            <option value="STARTS_WITH">시작</option>
            <option value="ENDS_WITH">끝</option>
          </select>
          <input bind:value={text} maxlength="100" placeholder="단어 입력" class="w-full min-w-0 flex-1 bg-transparent px-3 py-2.5 outline-none dark:text-white" />
        </div>
      </label>

      <div class="grid min-w-0 grid-cols-1 gap-2 sm:grid-cols-3">
        <label class="grid min-w-0 gap-1 text-xs font-bold text-slate-500">
          시작
          <input bind:value={start} maxlength="1" class="w-full min-w-0 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-center text-base font-black outline-none focus:border-emerald-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
        </label>
        <label class="grid min-w-0 gap-1 text-xs font-bold text-slate-500">
          끝
          <input bind:value={end} maxlength="1" class="w-full min-w-0 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-center text-base font-black outline-none focus:border-emerald-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
        </label>
        <label class="grid min-w-0 gap-1 text-xs font-bold text-slate-500">
          미션
          <input bind:value={mission} maxlength="1" class="w-full min-w-0 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-center text-base font-black outline-none focus:border-emerald-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
        </label>
      </div>

      <label class="grid min-w-0 gap-1.5 text-sm font-bold text-slate-600 dark:text-slate-300">
        정렬
        <select bind:value={sort} class="w-full min-w-0 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 outline-none dark:border-slate-700 dark:bg-slate-800 dark:text-white">
      ...
      <div class="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] gap-2">
        <button type="submit" disabled={loading} class="inline-flex min-h-11 min-w-0 items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 font-black text-white transition hover:bg-emerald-700 disabled:opacity-50">
  ...
  <section class="min-w-0">
    ...
    {#if error}
      <div class="mb-4 break-words rounded-2xl border border-rose-200 bg-rose-50 p-4 text-sm font-semibold text-rose-700 dark:border-rose-900 dark:bg-rose-950/40 dark:text-rose-300">{error}</div>
    {/if}
    ...
    {#if loading && result.items.length === 0}
      <div class="grid min-h-80 place-items-center rounded-3xl border border-dashed border-slate-300 bg-white/60 dark:border-slate-700 dark:bg-slate-900/60">
        <div class="text-center text-slate-500">
          ...
          <p class="mt-2 font-bold">검색 중입니다.</p>
        </div>
      </div>
    {:else if result.items.length === 0}
      <div class="grid min-h-80 place-items-center rounded-3xl border border-dashed border-slate-300 bg-white/60 p-8 text-center dark:border-slate-700 dark:bg-slate-900/60">
        <div>
          ...
          <h3 class="mt-3 text-lg font-black text-slate-700 dark:text-slate-200">검색 결과가 없습니다.</h3>
          <p class="mt-1 text-sm text-slate-500">검색 조건을 바꿔 다시 시도해 주세요.</p>
        </div>
      </div>
    {:else}
