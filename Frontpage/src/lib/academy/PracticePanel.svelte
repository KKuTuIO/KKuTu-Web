<script>
  ...
  function startTimer(seconds) {
    ...
    timer = setInterval(() => {
      ...
      if (timeLeft <= 0) {
        stopTimer();
        feedback = { accepted: false, message: '시간이 끝났습니다.', bestMoves: [] };
        streak = 0;
      }
    }, 1000);
  }
  ...
  <section class="min-w-0 overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
    <div class="bg-gradient-to-br from-emerald-600 to-teal-700 p-6 text-white md:p-8">
      <div class="flex min-w-0 flex-wrap items-center justify-between gap-4">
        <h2 class="text-3xl font-black">루트전 연습</h2>
    ...
    <div class="min-w-0 p-5 md:p-8">
        ...
        <div class="grid min-w-0 gap-5 md:grid-cols-[180px_minmax(0,1fr)]">
          ...
          <div class="min-w-0">
            <div class="flex min-w-0 flex-wrap items-center justify-between gap-3">
              <div class="min-w-0">
                <span class="rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-600 dark:bg-slate-800 dark:text-slate-300">{difficultyLabel(challenge.difficulty)}</span>
                <h3 class="mt-2 break-words text-xl font-black text-slate-900 dark:text-white">{challenge.objective}</h3>
              </div>
              {#if challenge.timeLimitSeconds > 0}
                <div class={`grid h-16 w-16 shrink-0 place-items-center rounded-full border-4 text-xl font-black ${timeLeft <= 3 ? 'border-rose-300 text-rose-600' : 'border-emerald-200 text-emerald-700 dark:border-emerald-900 dark:text-emerald-300'}`}>{timeLeft}</div>
              {/if}
            </div>

            <form class="mt-5 flex min-w-0 flex-col gap-3 sm:flex-row" onsubmit={submit}>
              <input bind:value={answer} disabled={!!feedback && !feedback.shieldUsed && !feedback.accepted} autocomplete="off" placeholder={`‘${challenge.requiredChar}’에서 이어지는 단어`} class="min-h-14 w-full min-w-0 flex-1 rounded-2xl border border-slate-200 bg-slate-50 px-4 text-lg font-black outline-none focus:border-emerald-500 disabled:opacity-50 dark:border-slate-700 dark:bg-slate-800 dark:text-white" />
              <button type="submit" disabled={loading || !answer.trim()} class="inline-flex min-h-14 shrink-0 items-center justify-center gap-2 rounded-2xl bg-emerald-600 px-7 font-black text-white hover:bg-emerald-700 disabled:opacity-50"><span class="material-symbols-outlined">send</span>제출</button>
            </form>
            ...
            <div class="mt-4 flex min-w-0 flex-wrap items-center gap-2">
              ...
              <button type="button" onclick={() => loadChallenge()} class="sm:ml-auto inline-flex items-center gap-1 rounded-xl border border-slate-200 px-3 py-2 text-sm font-bold text-slate-500 dark:border-slate-700"><span class="material-symbols-outlined text-lg">skip_next</span>다른 문제</button>
            </div>
            ...
            {#if hintOpen}
              <div class="mt-3 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-200">
                ...
                {#if !challenge.hint.sample && !challenge.hint.firstLetter && !challenge.hint.length}<p>힌트가 없습니다.</p>{/if}
              </div>
            {/if}
          </div>
        </div>
      ...
      {#if feedback}
        <div class={`mt-5 rounded-2xl border p-4 ${feedback.accepted ? 'border-emerald-200 bg-emerald-50 dark:border-emerald-900 dark:bg-emerald-950/30' : 'border-rose-200 bg-rose-50 dark:border-rose-900 dark:bg-rose-950/30'}`}>
          <p class="break-words font-black text-slate-800 dark:text-white">{feedback.message}</p>
          {#if !feedback.accepted && feedback.bestMoves?.length}
            <p class="mt-2 text-xs font-bold text-slate-500">가능했던 수</p>
            <div class="mt-2 flex min-w-0 flex-wrap gap-2">{#each feedback.bestMoves as move}<button type="button" onclick={() => answer = move.word} class="max-w-full break-all rounded-lg bg-white px-3 py-1.5 text-sm font-bold shadow-sm dark:bg-slate-800">{move.word} → {move.to}</button>{/each}</div>
          {/if}
  ...
  <aside class="grid h-fit min-w-0 gap-4 xl:sticky xl:top-28">
    <section class="min-w-0 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
      ...
      <div class="mt-4 grid min-w-0 gap-4">
        <label class="grid min-w-0 gap-1 text-sm font-bold text-slate-600 dark:text-slate-300">난이도
          <select bind:value={difficulty} onchange={restart} class="w-full min-w-0 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 dark:border-slate-700 dark:bg-slate-800 dark:text-white">
            <option value="BEGINNER">입문 · 무제한 시간</option>
            <option value="STANDARD">표준 · 15초</option>
            <option value="EXPERT">실전 · 8초</option>
          </select>
        </label>
        <label class="grid min-w-0 gap-1 text-sm font-bold text-slate-600 dark:text-slate-300">시작 음절 <span class="text-xs font-normal text-slate-400">비우면 자동 선택</span>
    ...
    <section class="min-w-0 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
      ...
      {#if history.length}
        <ol class="mt-3 grid max-h-96 min-w-0 gap-2 overflow-y-auto pr-1">
          {#each history.toReversed() as item}
            <li class="flex min-w-0 items-center justify-between gap-3 rounded-xl bg-slate-50 px-3 py-2 dark:bg-slate-800">
              <div class="min-w-0"><span class={`mr-2 text-xs font-black ${item.role === '봇' ? 'text-violet-500' : 'text-emerald-600'}`}>{item.role}</span><strong class="break-all dark:text-white">{item.word}</strong></div><span class="shrink-0 text-xs text-slate-400">{item.from}→{item.to}</span>
            </li>
          {/each}
        </ol>
      {:else}<p class="mt-3 text-sm text-slate-400">아직 기록이 없습니다.</p>{/if}
    </section>
  </aside>
</div>
