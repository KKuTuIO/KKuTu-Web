...
<section class="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-900">
  <div class="bg-gradient-to-r from-amber-400 via-orange-400 to-rose-400 p-6 text-white md:p-8">
    <div class="flex flex-wrap items-center justify-between gap-5">
      <div><p class="text-sm font-black text-white/80">DAILY TRAINING</p><h2 class="mt-1 text-3xl font-black">오늘의 낱말 퀴즈</h2><p class="mt-2 text-sm text-white/90">유효 단어, 다음 글자와 공격 수를 매일 짧게 복습합니다.</p></div>
      <div class="flex gap-3"><div class="rounded-2xl bg-white/20 px-5 py-3 text-center backdrop-blur"><span class="block text-xs">오늘 점수</span><strong class="text-2xl">{score}/{total}</strong></div><div class="rounded-2xl bg-white/20 px-5 py-3 text-center backdrop-blur"><span class="block text-xs">연속 학습</span><strong class="text-2xl">{streak}일</strong></div></div>
    </div>
  </div>
  ...
  <div class="p-5 md:p-8">
    ...
    {#if error}<p class="mb-4 rounded-xl bg-rose-50 p-3 text-sm font-bold text-rose-700 dark:bg-rose-950/40 dark:text-rose-300">{error}</p>{/if}
    ...
    {:else if question}
      <div class="mx-auto max-w-3xl">
        <div class="flex items-center justify-between"><span class="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-700 dark:bg-amber-950/50 dark:text-amber-300">{question.type}</span><span class="text-sm font-bold text-slate-400">{index + 1} / {total}</span></div>
        ...
        <div class="mt-8 grid gap-3 sm:grid-cols-2">
          {#each question.options as option, optionIndex}
            <button type="button" onclick={() => answer(option)} disabled={!!result || loading} class={`group flex min-h-20 items-center gap-4 rounded-2xl border p-4 text-left transition ${result && option === result.answer ? 'border-emerald-400 bg-emerald-50 dark:bg-emerald-950/30' : result && option === selected && !result.correct ? 'border-rose-400 bg-rose-50 dark:bg-rose-950/30' : 'border-slate-200 bg-slate-50 hover:-translate-y-0.5 hover:border-amber-400 hover:bg-amber-50 dark:border-slate-700 dark:bg-slate-800 dark:hover:bg-amber-950/20'} disabled:cursor-default`}>
              ...
              <strong class="break-all text-lg text-slate-800 dark:text-white">{option}</strong>
              {#if result && option === result.answer}<span class="material-symbols-outlined ml-auto text-emerald-500">check_circle</span>{/if}
              {#if result && option === selected && !result.correct}<span class="material-symbols-outlined ml-auto text-rose-500">cancel</span>{/if}
            </button>
          {/each}
        </div>
    ...
    {:else}
      <div class="grid min-h-96 place-items-center text-center"><div><span class="material-symbols-outlined text-6xl text-amber-400">workspace_premium</span><h3 class="mt-3 text-2xl font-black text-slate-900 dark:text-white">오늘 학습을 마쳤습니다.</h3><p class="mt-2 text-slate-500">{score}/{total}문제를 맞혔습니다. 내일 새로운 문제가 준비됩니다.</p><button type="button" onclick={restart} class="mt-5 rounded-xl border border-slate-200 px-5 py-2.5 font-black text-slate-600 dark:border-slate-700 dark:text-slate-300">다시 풀기</button></div></div>
    {/if}
  </div>
</section>
