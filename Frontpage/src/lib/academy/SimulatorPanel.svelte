<script>
  ...
  async function expertPick() {
    ...
    try {
      ...
      if (!suggestion) error = '현재 음절에서 추천할 수 있는 공개 단어가 없습니다.';
    } catch (cause) {
      error = friendlyError(cause);
    } finally {
      loading = false;
...
<div class="grid gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
  <section class="min-w-0 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900 md:p-7">
    <div class="flex flex-wrap items-start justify-between gap-4">
      <div>
        <p class="text-sm font-black text-emerald-600 dark:text-emerald-400">WORD CHAIN LAB</p>
        <h2 class="mt-1 text-2xl font-black text-slate-900 dark:text-white">끝말잇기 수순 시뮬레이터</h2>
        <p class="mt-1 text-sm text-slate-500">한 수씩 입력하며 연결, 중복, 두음법칙과 다음 선택지를 실험합니다.</p>
      </div>
    ...
    <div class="mt-6 min-h-44 rounded-3xl bg-gradient-to-br from-slate-50 to-emerald-50/60 p-4 dark:from-slate-950 dark:to-emerald-950/20">
      {#if chain.length === 0}
        <div class="grid min-h-36 place-items-center text-center">
          <div>
            ...
            <p class="mt-2 font-black text-slate-700 dark:text-slate-200">첫 단어를 입력하면 수순이 시작됩니다.</p>
            <p class="text-sm text-slate-500">URL의 start·dictionary 매개변수로 시작 상태를 공유할 수도 있습니다.</p>
          </div>
        </div>
      {:else}
        <div class="flex flex-wrap items-center gap-2">
          {#each chain as word, index}
            <div class={`rounded-2xl border px-4 py-3 shadow-sm ${autoBot && index % 2 === 1 ? 'border-violet-200 bg-violet-50 text-violet-800 dark:border-violet-900 dark:bg-violet-950/50 dark:text-violet-200' : 'border-emerald-200 bg-white text-slate-800 dark:border-emerald-900 dark:bg-slate-900 dark:text-white'}`}>
              <span class="block text-[10px] font-black uppercase tracking-wider opacity-50">{autoBot && index % 2 === 1 ? 'BOT' : `TURN ${index + 1}`}</span>
              <strong class="text-lg">{word}</strong>
            </div>
    ...
    <form class="mt-5" onsubmit={submit}>
      <div class="flex flex-col gap-3 sm:flex-row">
        <div class="relative flex-1">
          ...
          {/if}
          <input bind:value={input} autocomplete="off" placeholder={requiredChar ? `‘${requiredChar}’에서 이어지는 단어` : '첫 단어를 입력하세요'} class={`w-full rounded-2xl border border-slate-200 bg-slate-50 py-4 pr-4 text-lg font-black outline-none transition focus:border-emerald-500 dark:border-slate-700 dark:bg-slate-800 dark:text-white ${requiredChar ? 'pl-16' : 'pl-4'}`} />
        </div>
        <button type="submit" disabled={loading || !input.trim()} class="inline-flex min-h-14 items-center justify-center gap-2 rounded-2xl bg-emerald-600 px-6 font-black text-white transition hover:bg-emerald-700 disabled:opacity-50">
          <span class={`material-symbols-outlined ${loading ? 'animate-spin' : ''}`}>{loading ? 'progress_activity' : 'play_arrow'}</span>입력
        </button>
        <button type="button" onclick={expertPick} disabled={loading} class="inline-flex min-h-14 items-center justify-center gap-2 rounded-2xl bg-slate-900 px-5 font-black text-white transition hover:bg-slate-700 disabled:opacity-50 dark:bg-white dark:text-slate-900">
    ...
    {#if error}
      <p class="mt-3 rounded-xl bg-rose-50 px-4 py-3 text-sm font-bold text-rose-700 dark:bg-rose-950/40 dark:text-rose-300">{error}</p>
    {/if}
    {#if result}
      <div class={`mt-4 rounded-2xl border p-4 ${result.accepted ? 'border-emerald-200 bg-emerald-50 dark:border-emerald-900 dark:bg-emerald-950/30' : 'border-amber-200 bg-amber-50 dark:border-amber-900 dark:bg-amber-950/30'}`}>
        ...
        {#if result.analysis}
          <p class="mt-2 text-sm text-slate-600 dark:text-slate-300">다음 글자 <strong>{result.analysis.to}</strong> · 방어 {result.analysis.defenseCount}개 · 상태 {result.analysis.resultingState}</p>
        {/if}
      </div>
    {/if}

    {#if suggestion}
      <button type="button" onclick={() => input = suggestion.word} class="mt-4 w-full rounded-2xl border border-violet-200 bg-violet-50 p-4 text-left transition hover:border-violet-400 dark:border-violet-900 dark:bg-violet-950/30">
        <span class="text-xs font-black text-violet-600 dark:text-violet-300">고수픽 추천</span>
        <div class="mt-1 flex items-end justify-between gap-3">
          <strong class="text-xl text-slate-900 dark:text-white">{suggestion.word}</strong>
          <span class="text-sm text-slate-500">{suggestion.to} · 방어 {suggestion.defenseCount}개 · {suggestion.resultingState}</span>
        </div>
      </button>
    {/if}
      ...
      <div class="mt-5">
        ...
        <div class="mt-2 flex flex-wrap gap-2">
          {#each result.alternatives as move}
            <button type="button" onclick={() => input = move.word} class="rounded-xl border border-slate-200 bg-white px-3 py-2 text-left text-sm transition hover:border-emerald-400 dark:border-slate-700 dark:bg-slate-800">
              <strong class="text-slate-800 dark:text-white">{move.word}</strong>
  ...
  <aside class="grid h-fit gap-4 xl:sticky xl:top-28">
    <section class="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-700 dark:bg-slate-900">
      ...
      <div class="mt-4 grid gap-4">
        <label class="flex items-center justify-between rounded-xl bg-slate-50 px-3 py-3 text-sm font-bold text-slate-700 dark:bg-slate-800 dark:text-slate-200">
          봇 자동 응답
          <input type="checkbox" bind:checked={autoBot} class="h-4 w-4 accent-emerald-600" />
        </label>
        <label class="grid gap-1 text-sm font-bold text-slate-600 dark:text-slate-300">
          봇 선택 방식
          <select bind:value={botLevel} disabled={!autoBot} class="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 disabled:opacity-40 dark:border-slate-700 dark:bg-slate-800 dark:text-white">
        ...
        <div class="rounded-2xl bg-sky-50 p-4 dark:bg-sky-950/30">
          ...
          <p class="mt-1 text-xs leading-5 text-sky-600 dark:text-sky-400">잘못된 입력 한 번을 수순에 추가하지 않고 방어합니다.</p>
        </div>
      </div>
    </section>
