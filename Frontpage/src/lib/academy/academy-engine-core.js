...
export class AcademyLocalEngine {
  ...
  practiceAnswer({ config, difficulty = 'STANDARD', requiredChar, usedWords = [], word: text, shields = 0 }) {
    const view = this.view(config);
    ...
    const nextChallenge = view.destination(candidate);
    const usedAfterPlayer = [...usedWords, candidate.word];
    const graph = this.graph(config);
    const botMove = graphMoveViews(graph, nextChallenge, new Set(usedAfterPlayer))[0] || null;
    const usedAfterBot = botMove ? [...usedAfterPlayer, botMove.word] : usedAfterPlayer;
    let followingChallenge = null;
    if (botMove) {
      try {
        followingChallenge = this.practice({ config, difficulty, startChar: botMove.to, usedWords: usedAfterBot });
      } catch (error) {
        if (error?.code !== 'NO_CHALLENGE') throw error;
      }
    }
    return {
      accepted: true,
