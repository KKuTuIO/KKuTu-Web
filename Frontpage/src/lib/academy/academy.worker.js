...
async function fetchEngine(lang) {
...
function loadEngine(lang) {
  const normalized = lang === 'en' ? 'en' : 'ko';
  const cached = loaded.get(normalized)?.engine;
  if (cached && Date.now() - (manifestCheckedAt.get(normalized) || 0) < MANIFEST_RECHECK_MS) return Promise.resolve(cached);
  const pending = loading.get(normalized);
  if (pending) return pending;
  const task = fetchEngine(normalized).finally(() => loading.delete(normalized));
