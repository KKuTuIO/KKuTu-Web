...
<div class="dark:bg-gray-900">
    ...
    <div class="max-w-screen-xl mx-auto lg:py-12 p-4 lg:px-8 gap-y-12 flex flex-col">
        ...
        <div class="dark:text-white rounded-full p-2 flex flex-col">
            ...
            <div class="min-h-48 grid grid-cols-1 lg:grid-cols-4 gap-4">
                {#each finalData as cafeNotice, index}
                <!-- Card -->
                <a href={`https://cafe.naver.com/kkutuio/${cafeNotice.refArticleId}`} class="dark:text-white border flex flex-col" target="_blank">
                    <img src={`https://cdn.kkutu.io/img/front/${cafeNotice.menuId}.png`} class="h-32 w-full object-cover" alt="Patch note"/>
                    <h3 class="mt-4 px-3 pt-2 truncate">{cafeNotice.subject}</h3>
                    <p class="text-gray-400 text-sm px-3 pb-2">{tsconv(cafeNotice.writeDateTimestamp)}</p>
                </a>
                {/each}
            </div>
        </div>
