<script>
    ...
    import {uniqueProfiles} from '$lib/profiles.js';
    ...
    async function copyValue(identifier, label = '식별번호') {
        ...
        try {
            ...
            notify(`${label}(이)가 클립보드에 복사되었습니다.`, 'info');
        } catch (_) {
            notify(`${label}(을)를 복사하지 못했습니다.`, 'error');
        }
    }
    ...
    async function copyAccountIdentifier() {
        await copyValue(accountIdentifier, '계정 식별번호');
    }
    ...
    async function confirmPasskeyRename() {
        ...
        if (await call(`/api/account/passkeys/${modalPasskeyId}`, {
            method: 'PATCH',
    ...
    async function confirmTotp() {
        ...
        if (await call('/api/account/mfa/totp/confirm', {
            method: 'POST',
            body: JSON.stringify({code: modalTotpCode.trim()})
        })) {
            closeModal();
            totpSecret = '';
            await load();
...
<main class="min-h-screen bg-gray-100 px-4 pb-16 pt-24 text-gray-900 dark:bg-gray-900 dark:text-gray-100 sm:px-6">
    <div class="mx-auto max-w-3xl space-y-8">
        ...
        {#if summary}
            ...
            <section>
                <h2 class="mb-3 text-2xl font-bold text-gray-700 dark:text-gray-100">프로필 정보</h2>
                <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    <details open class="group border-b border-gray-200 dark:border-gray-700">
                        <summary class="flex cursor-pointer list-none items-center justify-between p-5 font-bold"><span>별명</span><span
                                class="material-symbols-outlined text-gray-500 transition group-open:rotate-180">expand_more</span>
                        </summary>
                        <div class="px-5 pb-5">
                            <div class="grid gap-2 sm:grid-cols-[1fr_auto]">
                                <div class="flex min-w-0"><input
                                        class={`min-w-0 flex-1 border border-gray-300 bg-white p-3 dark:border-gray-600 dark:bg-gray-900 ${fixedNickname ? 'rounded-xl' : 'rounded-l-xl'}`}
                                        maxlength="15" bind:value={nickname} placeholder="새 별명"/>
                                    {#if !fixedNickname}<span
                ...
                <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    <details class="group">
                        ...
                        <div class="px-5 pb-5">
                            ...
                            <div class="mt-3 grid gap-2 sm:grid-cols-[1fr_auto]"><input
                ...
                <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    {#if passwordEnabled}
                        <details class="group border-b border-gray-200 dark:border-gray-700">
                            ...
                            <div class="px-5 pb-5">
                                <div class="grid gap-2 sm:grid-cols-[1fr_auto]"><input
                                        class="rounded-xl border border-gray-300 bg-white p-3 dark:border-gray-600 dark:bg-gray-900"
                        ...
                        <div class="border-t border-gray-100 px-5 dark:border-gray-700">
                            {#each oauthProviders as provider}
                                ...
                                <div class="flex items-center gap-4 border-b border-gray-100 py-4 last:border-0 dark:border-gray-700">
                                    <img src={provider.icon} class="h-11 w-11 shrink-0 object-contain"
                    ...
                    <details class="group border-b border-gray-200 dark:border-gray-700">
                        ...
                        <div class="px-5 pb-5">
                            {#if passkeys.length === 0}
                                <div class="rounded-2xl border border-dashed border-gray-300 bg-gray-50 p-5 text-center dark:border-gray-600 dark:bg-gray-900">
                                ...
                                <div class="space-y-3">
                                    {#each passkeys as passkey}
                                        <div class="flex items-center gap-4 rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-600 dark:bg-gray-900">
                                            <div class="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-200">
                                                <span class="material-symbols-outlined text-3xl">key</span></div>
                                            <div class="min-w-0 flex-1">
                    ...
                    <details class="group border-b border-gray-200 dark:border-gray-700">
                        <summary class="flex cursor-pointer list-none items-center justify-between p-5 font-bold"><span>2단계 인증</span><span
                                class="flex items-center gap-3 text-sm font-normal text-gray-500">{mfa?.totp ? '사용 중' : '미설정'}
                            <span class="material-symbols-outlined transition group-open:rotate-180">expand_more</span></span>
                        </summary>
                        <div class="px-5 pb-5">
                            {#if mfa?.totp}
                                <div class="rounded-2xl border border-gray-200 bg-white p-4 dark:border-gray-600 dark:bg-gray-900">
                                    <div class="flex items-center gap-4">
                                        <div class="grid h-12 w-12 shrink-0 place-items-center rounded-xl bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-200">
                                            <span class="material-symbols-outlined text-3xl">qr_code_2</span></div>
                                        <div class="min-w-0 flex-1">
                                            <div class="flex items-center gap-1.5"><h3
                    ...
                    <details class="group border-b border-gray-200 dark:border-gray-700">
                        <summary class="flex cursor-pointer list-none items-center justify-between p-5 font-bold"><span>일회용 비밀번호</span><span
            ...
            <section>
                ...
                <div class="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800">
                    ...
                    <a class="flex items-center justify-between gap-3 border-t border-gray-200 p-5 font-bold transition hover:bg-gray-50 dark:border-gray-700 dark:hover:bg-gray-700"
                       href="/account/sanctions"><span>제재 내역</span><span
                            class="flex items-center gap-2 text-sm font-normal text-gray-500">보기<span
                            class="material-symbols-outlined">chevron_right</span></span></a>
                </div>
...
<AccountModal open={reauthDialogOpen} title="본인확인" showFooter={false} priority on:close={closeReauthentication}>
    ...
    {/if}
    {#if passwordEnabled && (reauthProviderIds.length > 0 || passkeys.length > 0)}<p
...
<AccountModal open={Boolean(modal)} title={modal?.title || ''}
    ...
    {:else if modal?.type === 'totp-setup'}
        ...
        {#if totpQr}<img
                ...
                src={totpQr} alt="2단계 인증 앱 등록 QR 코드"/>{/if}
    ...
    {:else if modal?.type === 'one-time-codes'}
        ...
        <div class="mt-4 grid grid-cols-2 gap-2">
            {#each recoveryCodes as code}<code
                    class="rounded-xl bg-amber-50 p-3 text-center font-bold text-amber-900 dark:bg-amber-950 dark:text-amber-100">{code}</code>{/each}
        </div>
    ...
    {:else if modal?.type === 'fixed-nickname-confirm'}
        <p class="text-sm leading-6 text-gray-600 dark:text-gray-300">100핑을 사용하여 별명을 고정합니다. 별명 고정 이후 180일 이상 게임에 접속하지
...
<style>
    ...
    :global(details[open]::details-content) {
        block-size: auto;
        opacity: 1;
    }

    :global(details[open] > :not(summary)) {
        animation: account-panel-open 180ms ease-out both;
    }
