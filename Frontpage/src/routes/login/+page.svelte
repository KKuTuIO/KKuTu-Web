<script nonce="kkutuio">
    ...
    async function passkeyLogin() {
        if (!window.PublicKeyCredential) { loginReason = '이 브라우저는 패스키를 지원하지 않습니다.'; return; }
        try {
            ...
            location.href = '/';
        } catch (_) { loginReason = 'Passkey 로그인에 실패했습니다. 다시 시도해 주세요.'; }
    }

    async function passwordLogin() {
        const response = await fetch('/api/account/password/login', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ identifier, password, totpCode: totpCode || null }) });
        if (response.ok) location.href = '/'; else loginReason = (await response.json().catch(() => ({}))).error_description || '로그인 정보가 올바르지 않습니다.';
    }
</script>
...
<div class="dark:bg-gray-900 flex min-h-screen flex-col justify-center px-6 py-12 lg:px-8">
    ...
    <div class="mt-4 sm:mx-auto sm:w-full ">
        <div class="sm:mx-auto sm:sm:max-w-sm">
            {#if passwordEnabled}<input class="mt-4 w-full border p-3 text-black" bind:value={identifier} placeholder="복구 메일 주소 또는 식별번호" autocomplete="username" />
            <input class="mt-2 w-full border p-3 text-black" bind:value={password} type="password" placeholder="비밀번호" autocomplete="current-password" />
            <input class="mt-2 w-full border p-3 text-black" bind:value={totpCode} inputmode="numeric" placeholder="2단계 인증 코드 (설정한 경우)" />
            <button class="text-lg bg-[#55aa55] text-white mt-2 flex w-full justify-center p-3 items-center font-semibold leading-6 shadow-md" on:click={passwordLogin}>전자 메일 주소로 로그인</button>{/if}
        ...
        <p class="mt-10 text-center text-sm text-gray-500 dark:text-gray-300">
            <a href="/account/recovery" class="link-signin">계정 정보를 분실하셨나요? 계정 복구하기 →</a><br/><br/>
