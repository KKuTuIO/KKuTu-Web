...
class AcademyDao(private val jdbcTemplate: JdbcTemplate) {
    ...
    fun loadPublicCorpus(lang: String): List<AcademyCorpusWord> {
        ...
        val classicWordClause = classicWordClause("w", lang)
    ...
    private fun classicWordClause(alias: String, lang: String): String = when (lang.lowercase()) {
        "ko" -> "CHAR_LENGTH($alias._id) >= 2 AND COALESCE($alias.type, '') ~ " +
            "'(^|,)(0|1|3|7|8|11|9|16|15|17|2|18|20|26|19|INJEONG)(,|$)'"
        "en" -> "CHAR_LENGTH($alias._id) >= 2 AND $alias._id ~* '^[a-z]+$'"
        else -> throw IllegalArgumentException("지원하지 않는 언어입니다.")
    }
