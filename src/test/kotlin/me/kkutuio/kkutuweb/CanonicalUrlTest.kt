    ...
    fun `layouts use model data instead of removed servlet expression objects`() {
        listOf("layout.html", "m_layout.html").forEach { template ->
            ...
            assertThat(source)
                ...
                .contains("href=\"/manifest.json\" th:href=\"@{/manifest.json}\"")
                .doesNotContain("#httpServletRequest")
                .doesNotContain("th:include")
        }
    }

    @Test
    fun `all template fragment expressions use the non-deprecated syntax`() {
        listOf(
            "layout.html",
            "m_layout.html",
            "view/kkutu/kkutu.html",
            "view/kkutu/m_kkutu.html"
        ).forEach { template ->
            val source = ClassPathResource("templates/$template").getContentAsString(Charsets.UTF_8)

            assertThat(source).doesNotContain("th:include")
            Regex("th:(?:insert|replace)=\\\"([^\\\"]+)\\\"")
                .findAll(source)
                .map { it.groupValues[1] }
                .filterNot { it.startsWith("~{") }
                .toList()
                .also { assertThat(it).describedAs("unwrapped fragments in $template").isEmpty() }
        }
    }
